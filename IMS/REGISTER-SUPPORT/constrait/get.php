	...
	if(isset($_POST['submit'])){
		...
		function admin()
		{
			try
			{
				function validate($data){
				 ...
				 require 'config.php';	
			  	 require 'uniq.php';
				 $autogen_reg = 'AD150'.get_rand_numbers(5).'';
				 $type = $_POST['role'];
				...
				$sqld = "INSERT INTO ims_admin_information(id,a_id,fname)values('$i_no','$i_no','$fname')";
		...
		function register_as_coordinator()
		{
			try
			{
				...
				$sql = "INSERT INTO ims_dummy_free_users(id,fullname,email,username,password,role)
									   			values('$i_no','$fname','$email','$autogen_reg','$password','$type')";
				$sqld = "INSERT INTO ims_department_information(id,id_number,firstname)values('$i_no','$autogen_reg','$fname')";
		...
		function register_as_DUMMY_student()
		{
			try
			{
				function validate($data){
				 ...
				 require 'config.php';	
			  	 require 'uniq.php';
				 $autogen_reg = 'S'.$current_year.'01'.get_rand_numbers(4).'';
				 $type = $_POST['role'];
				 $date = date('d-m-Y h:i A ');
				 $i_no = '10'.get_rand_numbers(3).'';
