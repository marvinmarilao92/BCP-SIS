...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  // Prepare an insert statement
        $sql = "INSERT INTO clearance_teacher_semester (name, school_year, date_started) VALUES (?, ?, ?)";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
              ...
              $sql = "SELECT * FROM clearance_teacher_status";
              if($result = mysqli_query($link, $sql)){
                if(mysqli_num_rows($result) > 0){
                  while($row = mysqli_fetch_array($result)){

                    // Prepare an insert statement
                    $sql1 = "INSERT INTO clearance_teacher_status_previous (clearance_requirement_id, location, teacher_id, status, clearance_department_id, date, file_link, remarks, semester_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

                    if($stmt1 = mysqli_prepare($link, $sql1)){
                        // Bind variables to the prepared statement as parameters
                        mysqli_stmt_bind_param($stmt1, "isisisssi", $param_clearance_requirement_id, $param_location, $param_teacher_id, $param_status, $param_clearance_department_id, $param_date, $param_file_link, $param_remarks, $param_semester_id);

                        // Set parameters
                        $param_clearance_requirement_id = $row['clearance_requirement_id'];
                        $param_location = $row['location'];
                        $param_teacher_id = $row['teacher_id'];
                        $param_status = $row['status'];
                        $param_clearance_department_id = $row['clearance_department_id'];
                        $param_date = $row['date'];
                        $param_file_link = $row['file_link'];
                        $param_remarks = $row['remarks'];
                        $param_semester_id = $id-1;
              ...
              $sql = "UPDATE clearance_teacher_semester_current SET semester_id=? WHERE id=?";
                  ...
                  if(mysqli_stmt_execute($stmt)){
                    ...
                    if($result = mysqli_query($link, $sql)){
                      ...
                      if($stmt1 = mysqli_prepare($link, $sql)){
                        ...
                        header("location: teacher-clearance.php");
                        exit();
                      }
                    } else{
                        echo "Oops! Something went wrong. Please try again later.";
...
?>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>Start New Clearance for teachers</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="index.php">Home</a></li>
          <li class="breadcrumb-item"><a href="teacher-clearance.php">teachers Clearance Status</a></li>
          <li class="breadcrumb-item">Start New Clearance for teachers</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
            ...
            <div class="card-body">
              ...
              <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                <div class="alert alert-primary alert-dismissible fade show" role="alert">  
                  <h4 class="alert-heading">An Email Notification will be send to all of teachers informing that the new clearance has started.</h4>
                  <p>
                ...
                <div class="float-end mb-3">
                    ...
                    <a href="teacher-clearance.php"><button type="button" class="btn btn-secondary">Cancel</button></a>
                </div>
