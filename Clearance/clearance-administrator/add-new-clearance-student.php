  ...
  // Prepare an insert statement
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
              ...
              $sql = "SELECT * FROM clearance_student_status";
              if($result = mysqli_query($link, $sql)){
                if(mysqli_num_rows($result) > 0){
                  while($row = mysqli_fetch_array($result)){

                    // Prepare an insert statement
                    $sql1 = "INSERT INTO clearance_student_status_previous (clearance_requirement_id, location, student_id, status, clearance_department_id, date, file_link, remarks, semester_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

                    if($stmt1 = mysqli_prepare($link, $sql1)){
                        // Bind variables to the prepared statement as parameters
                        mysqli_stmt_bind_param($stmt1, "isisisssi", $param_clearance_requirement_id, $param_location, $param_student_id, $param_status, $param_clearance_department_id, $param_date, $param_file_link, $param_remarks, $param_semester_id);

                        // Set parameters
                        $param_clearance_requirement_id = $row['clearance_requirement_id'];
                        $param_location = $row['location'];
                        $param_student_id = $row['student_id'];
                  ...
                  if(mysqli_stmt_execute($stmt)){
                      ...
                      if(mysqli_num_rows($result) > 0){
                        while($row = mysqli_fetch_array($result)){
                          if($row['clearance_type'] != "File Submission Hard Copy (Original Copy must be submitted)" && $row['clearance_type'] != "File Submission Soft Copy (Can be submitted online) or Hard Copy (Optional to submit the original copy)"){
                            $temp_id = $row['id'];
                            // Prepare an insert statement
                            $sql = "DELETE FROM clearance_student_status WHERE clearance_requirement_id = ?";
                            if($stmt = mysqli_prepare($link, $sql)){
                              // Bind variables to the prepared statement as parameters
                    ...
                    if($result = mysqli_query($link, $sql)){
                      ...
                      if($stmt1 = mysqli_prepare($link, $sql)){
                        // Bind variables to the prepared statement as parameters
                        $action = "Start new clearance for students named: '" . $semester . " SY: " . $school_year . "'";
