...
<script type="text/javascript">
  ...
  function OpenBootstrapPopup() {
    $("#students-record-modal-view").modal('show');
  }
</script>
<?php
}
...
?>
                      ...
                      echo "<tbody>";
                        while($row = mysqli_fetch_array($result)){
                          echo "<tr>";
                            ...
                            $sql = "SELECT * FROM clearance_students_record where department_name = '".$row['department_name']."'";
  ...
  <div class="modal fade" id="students-record-modal-view" tabindex="-1">
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">List of Students Record</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <?php
          // Attempt select query execution
          $sql = "SELECT * FROM clearance_students_record where department_name = '".$department_name."'";
          if($result = mysqli_query($link, $sql)){
              if(mysqli_num_rows($result) > 0){
                  echo '<table id="example" class="table datatable">';
                      echo "<thead>";
