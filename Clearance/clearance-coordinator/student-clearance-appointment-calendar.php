...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["date"]) && !empty($_POST["date"])){
  ...
  if($result = mysqli_query($link, $sql)){
    ...
    $sql = "SELECT * FROM clearance_student_appointment_specific where department = '$verified_session_role' and department_id = '$dept_id' and date = '$input_date' LIMIT 1";
    if($result = mysqli_query($link, $sql)){
      if(mysqli_num_rows($result) > 0){
        while($row = mysqli_fetch_array($result)){
          ...
          if($stmt = mysqli_prepare($link, $sql)){
              ...
              $param_department = $verified_session_role;
  ...
  if($result = mysqli_query($link, $sql)){
    ...
    $sql = "SELECT * FROM clearance_student_appointment_limit where department = '$verified_session_role' and department_id = '$dept_id' LIMIT 1";
    if($result = mysqli_query($link, $sql)){
      if(mysqli_num_rows($result) > 0){
        while($row = mysqli_fetch_array($result)){
...
$appointment_limit = 0;
$appointment_value = 0;
$sql = "SELECT * FROM clearance_student_appointment_limit where department = '$verified_session_role' LIMIT 1";
if($result = mysqli_query($link, $sql)){
  if(mysqli_num_rows($result) > 0){
    while($row = mysqli_fetch_array($result)){
...
for($day = 1; $day <= $day_count; $day++, $str++){
  ...
  $sql = "SELECT * FROM clearance_student_appointment_specific where department = '$verified_session_role' and date = '$date' LIMIT 1";
  if($result = mysqli_query($link, $sql)){
    if(mysqli_num_rows($result) > 0){
      while($row = mysqli_fetch_array($result)){
        $appointment_limit = $row['appointment'];
  ...
  if($str != 6 && $str != 7 && $str != 13 && $str != 14 && $str != 20 && $str != 21 && $str != 27 && $str != 28 && $str != 34 && $str != 35){
    ...
    $sql1 = "SELECT * FROM clearance_student_appointment where department = '$verified_session_role' and appointment_date = '" . $date . "'";
