...
$jobPost = $db->query('SELECT * FROM gac_jobpost WHERE ID=?', $_GET["deletePost"] )->fetchAll();
foreach ($jobPost as $post) { ?>
  <div class="form-row animate__animated animate__fadeOut" style="margin-bottom: 20px; ">
...
<?php  }?>
<?php
$deletePost = $db->query('DELETE FROM gac_jobpost WHERE ID=?', $_GET["deletePost"]);
 ?>
