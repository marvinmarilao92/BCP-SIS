    ...
    if(isset($_POST['docs_id'])&&isset($_POST['docs_code'])&&isset($_POST['docs_act2']) && isset($_POST['docs_off2'])){
        ...
        $q_checkcode = $conn->query("SELECT * FROM `datms_documents` WHERE `doc_code` = '$doc_code'") or die(mysqli_error($conn));
            ...
            if($v_checkcode < 1){
                ...
                $conn->query("UPDATE datms_documents SET doc_status = 'Received', doc_actor2 ='$d_act2', doc_off2='$d_off2', doc_date2 ='$date' , doc_actor3 ='', doc_off3 ='', doc_date3 ='', doc_remarks ='' WHERE doc_id='$id'") or die(mysqli_error($conn));
                echo ('success');
            }
        }else{
            echo ('Val69');
