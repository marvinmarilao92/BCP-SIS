...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["date"]) && !empty($_POST["date"])){
  ...
  $sql = "SELECT * FROM clearance_department_students where department_name = 'Registrar Coordinator'";
  if($result = mysqli_query($link, $sql)){
    ...
    $sql = "SELECT * FROM clearance_student_appointment_specific where department = 'Registrar Coordinator' and department_id = '$dept_id' and date = '$input_date' LIMIT 1";
    if($result = mysqli_query($link, $sql)){
      if(mysqli_num_rows($result) > 0){
        while($row = mysqli_fetch_array($result)){
          $sql = "UPDATE clearance_student_appointment_specific SET appointment=? WHERE department_id=? and department=? and date=?";
          ...
          if($stmt = mysqli_prepare($link, $sql)){
              ...
              if(mysqli_stmt_execute($stmt)){
                  ...
                  header("location: student-clearance-appointment-calendar.php");
                  exit();
              } else{
                  echo "Oops! Something went wrong. Please try again later.";
              }
          ...
          $sql = "INSERT INTO clearance_student_appointment_specific (appointment, department_id, department, date) VALUES (?, ?, ?, ?)";
  ...
  if($result = mysqli_query($link, $sql)){
    ...
    $sql = "SELECT * FROM clearance_student_appointment_limit where department = 'Registrar Coordinator' and department_id = '$dept_id' LIMIT 1";
    if($result = mysqli_query($link, $sql)){
      if(mysqli_num_rows($result) > 0){
        while($row = mysqli_fetch_array($result)){
          $sql = "UPDATE clearance_student_appointment_limit SET appointment=? WHERE department_id=? and department=?";
          ...
          // Prepare an insert statement
          $sql = "INSERT INTO clearance_student_appointment_limit (appointment, department_id, department) VALUES (?, ?, ?)";
...
$appointment_limit = 0;
$appointment_value = 0;
$sql = "SELECT * FROM clearance_student_appointment_limit where department = 'Registrar Coordinator' LIMIT 1";
if($result = mysqli_query($link, $sql)){
  if(mysqli_num_rows($result) > 0){
    while($row = mysqli_fetch_array($result)){
...
for($day = 1; $day <= $day_count; $day++, $str++){
  ...
  $sql = "SELECT * FROM clearance_student_appointment_specific where department = 'Registrar Coordinator' and date = '$date' LIMIT 1";
  if($result = mysqli_query($link, $sql)){
    if(mysqli_num_rows($result) > 0){
      while($row = mysqli_fetch_array($result)){
        $appointment_limit = $row['appointment'];
  ...
  if($str != 6 && $str != 7 && $str != 13 && $str != 14 && $str != 20 && $str != 21 && $str != 27 && $str != 28 && $str != 34 && $str != 35){
    ...
    $sql1 = "SELECT * FROM clearance_student_appointment where department = 'Registrar Coordinator' and appointment_date = '" . $date . "'";
      ...
      <div class="card-body">
        <div class="container">
          <ul class="pagination justify-content-center pagination-lg">
            ...
            <p class="text-end"><a href="student-clearance-appointment-calendar.php" class="btn btn-primary">Today</a></p>
