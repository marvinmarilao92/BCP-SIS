        ...
        <body>
                      ...
                      <div class="card-body">
                        ...
                        <ul class="nav nav-tabs nav-tabs-bordered d-flex" id="myTabjustified" role="tablist" style="margin-top: 10px;">
                          <li class="nav-item flex-fill" role="presentation">
                            <button class="nav-link w-100 active" id=" incoming-tab" data-bs-toggle="tab" data-bs-target="#IncomingDocs" type="button" role="tab" aria-controls="incoming" aria-selected="true">Personal Tracked Files</button>
                          </li>
                        </ul>
          ...
          <?php
              ...
              if (isset($_POST['save'])) { // if save button on the form is clicked
                    ...
                    $query  = "SELECT *,LEFT(middlename,1) as MI FROM student_information WHERE course = '".$verified_session_office."' AND id_number = '".$doc_title."'";
                    $result = mysqli_query($conn, $query);
                    if(mysqli_num_rows($result) > 0){
                      ...
                      $query  = "SELECT *,LEFT(middlename,1) as MI FROM teacher_information WHERE course = '".$verified_session_office."' AND`account_status` NOT IN ('Inactive') AND id_number = '".$doc_title."'";
                      $result = mysqli_query($conn, $query);
                      if(mysqli_num_rows($result) > 0){
                        if (!in_array($extension, ['pdf'])) {
                          echo'<script type = "text/javascript">
