    ...
    if(isset($_POST['docs_id'])&&isset($_POST['docs_code'])&&isset($_POST['docs_act2']) && isset($_POST['docs_off2'])){
        ...
        $q_checkcode = $conn->query("SELECT * FROM `datms_documents` WHERE `doc_code` = '$doc_code'") or die(mysqli_error($conn));
            ...
            if($v_checkcode < 1){
                ...
                $conn->query("UPDATE datms_documents SET doc_status = 'Pending', doc_actor2 ='$d_act2', doc_off2='$d_off2', doc_date2 ='$date' WHERE doc_id='$id'") or die(mysqli_error($conn));
                          // record for tracking
                          ...
                          if($result1 = mysqli_query($conn, $sql1)){
                              ...
                              while ($row1 = mysqli_fetch_array($result1)) {
                                  ...
                                  VALUES ('$doc_code', '$doc_title' ,'$filename','$size','$doc_type','Pending','$doc_desc','$d_act2','$d_off2','$date','$d_act1','$d_off1','$d_date1','Document is Recieved by')") or die(mysqli_error($conn));
