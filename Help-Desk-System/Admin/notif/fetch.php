...
if(isset($_POST["view"]))
{
 ...
 if($_POST["view"] != '')
 {
  $update_query = "UPDATE datms_notification SET stat1 = 1 WHERE act1 = '1' AND stat1 = 0";
  mysqli_query($conn, $update_query);
 }
 $query = "SELECT * FROM datms_notification WHERE act1 = '1' ORDER BY date DESC LIMIT 10";
 ...
 if(mysqli_num_rows($result) > 0)
 {
  while($row = mysqli_fetch_array($result))
  {
      ...
      if ($doc_status ==''){
      ...
      }else if($doc_status =='staff forwarded ticket'){
        $idenifier=' <i class="bi bi-plus-circle text-primary"></i>';  
      }else{
       ...
       }else if($doc_status =='Staff forwarded a ticket to'){
        $links='admin-ticket-view.php?id='.$_SESSION["login_key"].''; 
       }else{
        $links='admin-ticket-view.php?id='.$_SESSION["login_key"].'';
       }
 ...
 $query_1 = "SELECT * FROM datms_notification WHERE act1 = '1' AND stat1 = 0";
