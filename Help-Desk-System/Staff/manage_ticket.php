...
<?php 
$qry = $conn->query("SELECT * FROM hdms_tickets where id = ".$_GET['id'])->fetch_array();
foreach($qry as $k => $v){
	$$k = $v;
}
?>
...
<script>
	$('#update-ticket').submit(function(e){
		...
		// $('#msg').html('')
		$.ajax({
			url:'function/ajax.php?action=update_ticket',
			...
			success:function(resp){
				if(resp == 1){
					alert_toast('Ticket successfully updated.',"success");
					setTimeout(function(){
						location.reload()
					},1500)
				}
