...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"]) && trim($_POST["action"])=="view-previous-clearance" && isset($_POST["semester_id"])){
  ...
  if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"]) && trim($_POST["action"])=="view-clearance" && isset($_POST["user"]) && isset($_POST["id"]) && isset($_POST["fullname"]) && isset($_POST["course"]) && isset($_POST["semester_id"])){
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"]) && trim($_POST["action"])=="view-description"  && isset($_POST["department_name"]) && isset($_POST["semester_id"])){
      $semester_id = trim($_POST["semester_id"]);
      ...
      if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"]) && trim($_POST["action"])=="print-clearance" && isset($_POST["semester_id"])){
          ...
          $content = "";
            $sql = "SELECT * FROM clearance_semestral_clearance_list where semester_id = '".$semester_id."' and id_number = '".$verified_session_username."' ORDER BY department_name";
            if($result = mysqli_query($link, $sql)){
              if(mysqli_num_rows($result) > 0){
                  while($row = mysqli_fetch_array($result)){
                    $content = $content.'
            ...
            <div class="row mx-5 mb-5 mt-1 border">
              ...
              <div class="float-end">
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                  ...
                  <a href="semestral-clearance.php" type="button" class="btn btn-info">Back to Current Semester</a>
                </form>
              </div>
            </div>
          </div>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body" id="clearance">
          <div class="row mx-5 mb-2">
            <div class="col-12"><span class="">Name: <?php echo $verified_session_firstname.' '.$verified_session_lastname; ?></span></div>
            ...
            <?php 
            $sql = "SELECT * FROM clearance_semestral_clearance_list where id_number = '".$verified_session_username."' and department_name = '".$department_name."' and semester_id = '".$semester_id."' LIMIT 1";
            if($result = mysqli_query($link, $sql)){
              if(mysqli_num_rows($result) > 0){
                while($row = mysqli_fetch_array($result)){
