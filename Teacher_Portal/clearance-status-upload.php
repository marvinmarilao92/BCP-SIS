        ...
        if($fileError === 0){
          if($fileSize < 52428800){
            ...
            if ($temp_status == "Declined") {
                $sql1 = "SELECT * FROM clearance_teacher_status where clearance_requirement_id = '$req_id' and teacher_id = '$verified_session_username' LIMIT 1";
                if($result1 = mysqli_query($link, $sql1)){
                  if(mysqli_num_rows($result1) > 0){
                    while($row1 = mysqli_fetch_array($result1)){
                      $path = 'uploads/' . $row1['file_link'];
                      if(unlink($path)){
                        $sql3 = "DELETE FROM clearance_teacher_status where clearance_requirement_id = '$req_id' and teacher_id = '$verified_session_username' LIMIT 1";
                        $result3 = mysqli_query($link,$sql3);
                        if(move_uploaded_file($fileTmpName, $fileDestination)){

                          $sql = "INSERT INTO clearance_teacher_status (clearance_requirement_id, location, teacher_id, status, clearance_department_id, file_link) VALUES ('$req_id', '$location', '$verified_session_username', '$status', '$id', '$fileNameNew')";
