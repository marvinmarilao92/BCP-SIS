...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  $sql1 = "SELECT * FROM clearance_student_appointment where student_id = '$verified_session_username' and department_id = '". trim($_POST["id"]) ."' LIMIT 1";
  if($result1 = mysqli_query($link, $sql1)){
    if(mysqli_num_rows($result1) > 0){
      while($row1 = mysqli_fetch_array($result1)){

        $sql = "DELETE FROM clearance_student_appointment WHERE student_id = ? and department_id = ?";

        if($stmt = mysqli_prepare($link, $sql)){
          // Bind variables to the prepared statement as parameters
          mysqli_stmt_bind_param($stmt, "ii", $verified_session_username, trim($_POST["id"]));

          // Attempt to execute the prepared statement
          if(mysqli_stmt_execute($stmt)){

            $sql2 = "INSERT INTO clearance_student_appointment (student_id, department_id, appointment_date, department, appointment_id) VALUES (?, ?, ?, ?, ?)";
            if($stmt2 = mysqli_prepare($link, $sql2)){
                // Bind variables to the prepared statement as parameters
                mysqli_stmt_bind_param($stmt2, "iisss", $param_student_id, $param_department_id, $param_appointment_date, $param_department, $param_appointment_id);

                // Set parameters
                $param_student_id = $verified_session_username;
